...
from utils import print_trainable_params, labels_to_one_hot, filter_samples, replace_cls_label, random_remove_samples
...
def arg_parse():
    ...
    parser.add_argument('--mlp', type=str, default="140,512,5",
                        help="mlp for model before continue learning")
    ...
    parser.add_argument('--num_selected_speakers_pretrained', type=int, default=5, 
                        help='number of speakers selected for pretrained model')
    parser.add_argument('--num_selected_speakers_continue', type=int, default=7, 
                        help='number of speakers selected for continue learning')
    ...
    ## checkname
    parser.add_argument('--name',type=str, default='snn_lsm_512_full_continue_5cls_10tw')
    ...
    parser.add_argument('--pretrained_model', type=str, default='./checkpoints_randlabel_unlearn_full/snn_lsm_512_full_randlabel_5cls_10twat51.pth', help='Path to the pretrained model')
...
os.makedirs('checkpoints_continue_full', exist_ok=True) 
log_file = open(f'checkpoints_continue_full/{args.name}_log.txt', 'w')

with open(f'checkpoints_continue_full/{args.name}_config.txt', 'w') as config_file:
...
if args.previous_unlearn_idx is not None:
...
for cls_idx, cls_acc in cls_accs_dict.items():
    ...
    with open(f'checkpoints_continue_full/{args.name}_log.txt', 'a') as log_file:
...
for epoch in range(n_epochs):
    ...
    for i, data in enumerate(train_loader):
        ...
        random_inputs, random_labels = random_remove_samples(filtered_inputs, filtered_label_speakers, remove_ratio=0, class_idx_list=available_class_idx_pretrained, seed=args.seed, epoch=epoch)  
    ...
    if continue_cls_acc_test > 0.90 and other_cls_acc_test > 0.92:
        torch.save(model.state_dict(), f'checkpoints_continue_full/{args.name}at{epoch+1}.pth')
