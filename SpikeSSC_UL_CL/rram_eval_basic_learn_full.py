...
def arg_parse():
    ...
    ## checkname
    parser.add_argument('--name',type=str, default='snn_lsm_512_5cls_10tw_best_model')


    parser.add_argument('--pretrained_model', type=str, default='./checkpoints/snn_lsm_512_5cls_10tw_best_model.pth',
...
for k, v in weights.items():
    # Skipping bias terms (only processing weights)
    if 'bias' in k:  
    ...
    # Calculating positive and negative maxima
...
def evaluate_model_RRAM(model, test_loader, num_selected_speakers, unlearned_class_idx,
    ...
    # Returning overall accuracy, per-class accuracy dictionary, forgetting metric, and retention metric
...
print(f"Original class accuracies:")
...
csv_filename = f"noise_sensitivity_results_lr.csv"
...
summary_filename = f"./noise_sensitivity_summary_lr.csv"
...
for i in range(num_selected_speakers):
    ...
    impact_type = "Forgotten class" if i == 2 else f"Class {i}"
